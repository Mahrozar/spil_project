...
@section('title', 'Daftar RT (Rukun Tetangga)')
        ...
        <div class="md:flex md:items-center md:justify-between mb-8">
            <div class="flex-1 min-w-0">
                <h2 class="text-2xl font-bold leading-7 text-gray-900 sm:text-3xl sm:truncate">
                    Daftar RT (Rukun Tetangga)
                </h2>
                <p class="mt-1 text-sm text-gray-500">
                    Kelola data Rukun Tetangga di lingkungan Anda
                </p>
            </div>
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                ...
                                <dd class="text-lg font-medium text-gray-900">
                                    {{ $rts->whereNotNull('leader_name')->count() }}
                                </dd>
                            </dl>
                        </div>
                    </div>
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    <div class="w-full sm:w-1/2 mb-4 sm:mb-0">
                        <div class="relative">
                            ...
                            <input type="text" id="search" placeholder="Cari RT, Ketua RT, atau RW..."
                                class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                        </div>
                    </div>
                    <div class="flex items-center space-x-2">
                        <span class="text-sm text-gray-700">
                            Menampilkan {{ $rts->firstItem() ?? 0 }} - {{ $rts->lastItem() ?? 0 }} dari
                            {{ $rts->total() }} RT
                        </span>
                    </div>
                </div>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <div
                                            ...
                                            <span class="text-blue-600 font-bold">{{ substr($rt->name, 2) }}</span>
                                        </div>
                                        <div class="ml-4">
                                            <div class="text-sm font-medium text-gray-900">{{ $rt->name }}
                                            </div>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if ($rt->rw)
                                        <div class="flex items-center">
                                            <div
                                                class="flex-shrink-0 h-8 w-8 flex items-center justify-center bg-green-100 rounded-lg mr-2">
                                                <span
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if ($rt->leader_name)
                                        <div class="text-sm font-medium text-gray-900">{{ $rt->leader_name }}
                                        </div>
                                    @else
                                        <span
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        ...
                                        <form action="{{ route('admin.rts.destroy', $rt) }}" method="POST"
                                            class="inline delete-form" data-rt-name="{{ $rt->name }}">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-6 py-12 text-center">
                                    ...
                                    <p class="mt-1 text-sm text-gray-500">
                                        Mulai dengan menambahkan RT baru.
                                    </p>
                                    <div class="mt-6">
                                        <a href="{{ route('admin.rts.create') }}"
...
@push('scripts')
    ...
    <script>
        ...
        function confirmDelete(button) {
            ...
            const rtName = form.getAttribute('data-rt-name');

            Swal.fire({
                title: 'Hapus RT?',
                html: `Apakah Anda yakin ingin menghapus <strong>${rtName}</strong>?<br><br>
        ...
        function confirmDeleteFallback(button) {
            ...
            if (confirm(`Apakah Anda yakin ingin menghapus ${rtName}?\n\nData yang dihapus tidak dapat dikembalikan!`)) {
                form.submit();
            }
        }
    </script>
