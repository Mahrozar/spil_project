<x-app-layout>
    ...
    <div class="max-w-3xl mx-auto p-6">
        <h2 class="text-lg font-semibold mb-4">Penduduk Baru</h2>
        ...
        <form method="POST" action="{{ route('admin.residents.store') }}">
            ...
            <div class="grid grid-cols-1 gap-4">
                <div>
                    ...
                    <input name="nik" class="w-full border rounded px-3 py-2" value="{{ old('nik') }}" />
                </div>
                <div>
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div>
                        ...
                        <select name="rt_id" class="w-full border rounded px-2 py-2">
                            <option value="">--</option>
                            @foreach($rts as $rt)
                                <option value="{{ $rt->id }}">{{ $rt->name }} (RW {{ $rt->rw->name ?? '-' }})</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
